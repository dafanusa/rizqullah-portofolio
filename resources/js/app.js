...
const setupFilters = () => {
    ...
    const activeFilter = Array.from(filterButtons).find(
        (button) => button.getAttribute("data-active") === "true"
    );
    if (activeFilter) {
...
const setupCertificates = () => {
    ...
    const updateCertificatePreview = (previewSrc, fileSrc, title) => {
        ...
        const downloadName = `${(title || "certificate")
            .replace(/\s+/g, "-")
            .toLowerCase()}.${fileExt}`;
    ...
    const bindCertificateDownload = () => {
        if (
            !downloadLink ||
            downloadLink.getAttribute("data-bound") === "true"
        ) {
            return;
        }
    ...
    const ensurePdfPlaceholder = (card, previewSrc, fileSrc) => {
        ...
        placeholder.className =
            "absolute inset-0 flex flex-col items-center justify-center gap-2 bg-slate-950/70 text-amber-200";
        placeholder.innerHTML =
            '<span class="text-sm font-semibold uppercase tracking-[0.35em]">PDF</span><span class="text-[10px] uppercase tracking-[0.25em] text-slate-300">Click to preview</span>';
